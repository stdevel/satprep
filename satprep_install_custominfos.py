...
If you're not defining variables or an authfile you will be prompted to enter your login information.
	...
	resultcode=0
	if options.debug: print "DEBUG: pre-defined custom information keys:\n" + str(definedKeys)
	for newKey in customKeys:
		if options.debug: print "DEBUG: about to add system information key '" + newKey + "' with description '" + customKeys.get(newKey) + "'..."
		if newKey in str(definedKeys):
			if options.force == True:
				if options.verbose: print "INFO: overwriting pre-existing key '" + newKey + "' with description '" + customKeys.get(newKey) + "'..."
				resultcode = client.system.custominfo.updateKey(key, newKey, customKeys.get(newKey))
			else:
				print "ERROR: key '" + newKey + "' already exists. Use -f / --force to overwrite!"
		else:
			resultcode = client.system.custominfo.createKey(key, newKey, customKeys.get(newKey))
		if resultcode == 1:
			if options.verbose: print "INFO: successfully created/updated information key '" + newKey + "'"
		else:
			if newKey not in customKeys: print "ERROR: unable to create key '" + newKey + "': check your account permissions!"
